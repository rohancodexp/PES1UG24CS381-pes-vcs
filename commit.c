...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // 1. Create a tree snapshot from the current staging area
    // This writes the tree object and returns the root hash to commit.tree
    if (tree_from_index(&commit.tree) != 0) {
        fprintf(stderr, "error: failed to create tree from index\n");
        return -1;
    }

    // 2. Check if there is a parent commit (read from HEAD)
    // If head_read succeeds, it's not the first commit
    ...
    // 3. Fill in metadata
    // pes_author() is provided in pes.h
