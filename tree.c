...
int tree_from_index(ObjectID *id_out) {
    ...
    // 2. Map index entries to tree entries
    for (int i = 0; i < index.count && i < MAX_TREE_ENTRIES; i++) {
        TreeEntry *te = &tree.entries[tree.count++];
        te->mode = index.entries[i].mode;
        
        // Use .hash to match your index.h structure
        memcpy(te->hash.hash, index.entries[i].hash.hash, HASH_SIZE);
    ...
    // object_write is defined in pes.h/object.c
